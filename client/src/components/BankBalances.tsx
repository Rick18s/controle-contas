  ...
  return (
    <div className="rounded-3xl border border-white/5 bg-zinc-900 p-4 shadow-sm transition-all hover:border-purple-500/30 sm:rounded-2xl">
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        <Button onClick={() => setShowAddDialog(true)} size="sm" variant="ghost" className="h-10 shrink-0 gap-1 rounded-2xl text-xs text-primary hover:text-primary/80 sm:h-8">
      ...
      <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
        <DialogContent className="border border-border bg-card text-card-foreground sm:max-w-md">
          ...
          <DialogFooter className="grid grid-cols-1 gap-2 sm:flex">
            <Button variant="ghost" onClick={() => setShowAddDialog(false)} className="h-11 text-xs text-gray-400">Cancelar</Button>
            <Button onClick={() => { void handleAddBalance(); }} disabled={updateBalance.isPending} className="h-11 text-xs" >
...
function BalanceInput({ label, value, onSave, onDelete }: { label: string; value: string; onSave: (val: string) => void; onDelete: () => void }) {
  ...
  return (
    <div className="flex flex-col gap-2 rounded-2xl border border-white/5 bg-zinc-800/50 p-3 min-[380px]:flex-row min-[380px]:items-center min-[380px]:justify-between sm:rounded-lg sm:p-2">
      <span className="text-[11px] font-mono text-gray-400 uppercase tracking-wide flex-1">{label}</span>
      <div className="flex items-center justify-between gap-1 min-[380px]:justify-end">
        ...
        <input
          ...
          className="w-full min-w-0 border-none bg-transparent text-right text-base font-bold text-primary focus:outline-none focus:text-white min-[380px]:w-28 sm:text-sm"
        />
        <Button
          type="button"
          variant="ghost"
          size="sm"
          onClick={onDelete}
          className="h-9 w-9 p-0 text-gray-600 hover:text-red-400 sm:h-6 sm:w-6"
          title="Remover conta"
        >
