...
## Publicação com banco online

Para usar o app online com dados persistentes:

1. Crie um banco MySQL compatível.
2. Copie `.env.example` para `.env` e preencha `DATABASE_URL`, `JWT_SECRET`, `LOCAL_AUTH_USERNAME` e `LOCAL_AUTH_PASSWORD`.
3. Rode `pnpm db:setup-online` para criar/atualizar as tabelas e importar os dados atuais de `data/local-data.json`.
4. Rode `pnpm build` e depois `pnpm start`.

Depois que `DATABASE_URL` estiver configurado, o app deixa de usar o arquivo local e passa a salvar as alterações no banco online.

Use `pnpm db:import-local -- --replace` apenas quando quiser limpar o banco e reimportar o snapshot local do zero.
